...
// base file names
let cssOutputName = !prod
  ? `${config.outputFileName}.v${config.cssVersion}`
  : `${config.outputFileName}.v${config.cssVersion}.min`;
...
const devTasks = ["clearConsole", "watch"];
...
gulp.task(
  "dist",
  !prod ? commonTasks.concat(devTasks) : commonTasks.concat(prodTasks)
);
...
/* WATCH :: ALL */
gulp.task("watch", ["js:watch", "sass:watch", "html:watch"]);
...
gulp.task("sass", () => {
  var sourceFile = gulp
    .src([devPaths.scss_main])
    .pipe(sass().on("error", pingError))
    .pipe(
      autoprefixer({
...
gulp.task("jsES6", () => {
  var stream = gulp
    .src(devPaths.js_ES6, { read: false })
    .pipe(
      tap(file => {
...
gulp.task("html", ["compileHTML"], () => {
  var sourceFile = null;
  var cssPath = !prod
    ? `css/${cssOutputName}.css`
    : `${cssCDNPath}${cssOutputName}.css`;
  var jsPath = !prod
    ? `js/${jsOutputName}.js`
    : `${jsCDNPath}${jsOutputName}.min.js`;
  ...
  // set source file(s)
  sourceFile = gulp.src([`${distDir}/**/*.html`]);

  // replace html
  sourceFile = sourceFile.pipe(
    htmlreplace(htmlreplaceData, {
      keepUnassigned: true
    })
  );
